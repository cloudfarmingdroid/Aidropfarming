/**
 * Bam Mining App - Core Logic
 * ---------------------------
 * Handles auth, dashboard, activity claims, transfers, and UI state.
 */

const STORAGE_KEY = "bam_users_v1";
const SESSION_KEY = "bam_session_v1";
...
async function handleRegister(e) {
  ...
  // Process referral code if present (from URL ?ref=)
  const refCode = sessionStorage.getItem("bam_ref_code");
  let referredBy = null;
  if (refCode) {
    for (const u of Object.values(users)) {
      if (u.wallet && u.wallet.slice(2, 10).toLowerCase() === refCode.toLowerCase()) {
        u.referrals = (u.referrals || 0) + 1;
        // Optional small bonus for the referrer
        u.balance = (u.balance || 0) + 0.1;
        u.history = u.history || [];
        u.history.unshift({
          type: "referral",
          amount: 0.1,
    ...
    sessionStorage.removeItem("bam_ref_code");
  }
  ...
  showToast(referredBy ? "Welcome to Bam! (referred) 🚀" : "Welcome to Bam! 🚀", "success");
  setTimeout(() => {
    showApp();
  }, 600);
}
...
function claimActivity() {
  ...
  showToast(`+${ACTIVITY_REWARD} BAM claimed! 🎉`, "success");
  renderActivity();
  renderDashboard();
}
...
function handleSendPoints(e) {
  ...
  if (!recipient) {
    showToast("Recipient wallet not found on Bam", "error");
    return;
  }
  ...
  showToast(`Successfully sent ${amount} BAM! 🚀`, "success");
  renderWallet();
  renderDashboard();
}

function getReferralBaseUrl() {
  // Use current site origin + path (works on GitHub Pages and locally)
  const base = window.location.origin + window.location.pathname.replace(/\/index\.html$/, "/");
  return base.endsWith("/") ? base : base + "/";
}

function getReferralCode(user) {
  return user.wallet.slice(2, 10);
}
...
function renderReferrals() {
  ...
  const link = `${getReferralBaseUrl()}?ref=${getReferralCode(user)}`;
...
// ---------- Init ----------
function init() {
  // Capture referral code from URL (?ref=XXXXXXXX) and remember it
  try {
    const params = new URLSearchParams(window.location.search);
    const ref = params.get("ref");
    if (ref && /^[0-9a-fA-F]{6,12}$/.test(ref)) {
      sessionStorage.setItem("bam_ref_code", ref);
      // Clean the URL so the param doesn't stay visible / shareable accidentally
